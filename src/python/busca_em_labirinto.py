...
of finding a path from the "start" point to the "goal" point.
...
class Maze:
    ...
    def successors(self, ml):
        """Calculates the possible locations where it is possible to move."""
        locations = list()
    ...
    def mark(self, path):
        """Marks the walked path in the maze."""
    ...
    def clear(self, path):
        """Clears the marked path in the maze."""
        for maze_location in path:
...
def dfs(initial, goal_test, successors):
    ...
    # The "frontier" represents the locations that have not been visited yet.
    frontier = Stack()
    frontier.push(Node(initial, None))
    # "explored" represents the locations that have already been visited.
    explored = {initial}

    # Continues while there are places to explore.
    while not frontier.empty:
        current_node = frontier.pop()
        current_state = current_node.state

        # If it finds the goal, it returns the current node.
        if goal_test(current_state):
            return current_node

        # Checks where we can go next.
        for child in successors(current_state):
            # Ignores the child nodes that have already been visited.
            if child in explored:
                continue
            explored.add(child)
            frontier.push(Node(child, current_node))
    # We have passed through all the locations and have not reached the goal.
    return None
...
def node_to_path(node):
    """Returns the path found by the algorithm."""
    path = [node.state]
    while node.parent:
        node = node.parent

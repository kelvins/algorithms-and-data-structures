...
fun fibonacciMemorization(n: Int): Long {
    if (n <= 1) {
        return n.toLong()
    }
    ...
    val result = fibonacciMemorization(n - 1) + fibonacciMemorization(n - 2)
    memo[n] = result
...
fun main() {
    val index = 15
    println("Fibonacci (memoization) of $index is: ${fibonacciMemorization(index)}")
}
